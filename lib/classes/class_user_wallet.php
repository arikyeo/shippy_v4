...
class class_user_wallet extends class_main {
	...
	public function get_user_wallet_balance_amt_clovers($user_hash) {

		// Variables
		$total_clovers = 0.00;
		$total_topups = 0.00;
		$total_withdrawals = 0.00;
		$total_bet_payouts = 0.00;
		...
		try {
			...
			if ($result_topups['txn-amt_clovers']) {
				// The total amount of clovers
				$total_topups = $result_topups['txn-amt_clovers'];
			}
		...
		// Get all bet payouts from bet_ledger
		$query_bet_payouts = "SELECT
								SUM(`bet_payout`) AS `bet_payout`
							FROM bet_ledger
							WHERE `bet_player_hash` = :userhash
							";

		// The parameter values
		$query_params_bet_payouts = array(
			':userhash' => $user_hash,
		);

		// Execute the query
		try {
			$stmt_bet_payouts = $this->db->prepare($query_bet_payouts);
			$stmt_bet_payouts->execute($query_params_bet_payouts);
			$result_bet_payouts = $stmt_bet_payouts->fetch(PDO::FETCH_ASSOC);

			if ($result_bet_payouts['bet_payout']) {
				$total_bet_payouts = $result_bet_payouts['bet_payout'];
			}
		} catch (PDOException $e) {
			$this->add_error($e->getMessage(), 'error');
			return 0.00;
		...
		// Calculate the total clovers
		$total_clovers = $total_topups - $total_withdrawals + $total_bet_payouts;

		// Return the total amount of clovers
		return $total_clovers;
	}


	// Get the user's wallet balance details
	// 1. Available balance for spending = total_topups - total_withdrawals + total_bet_payouts
	// 2. Withdrawable balance = (total_bet_payouts - total_withdrawals) - total_topups (Any topped up clovers are not withdrawable)
