...
/* The user's wallet balance is broken down into a few categories:
	...
	1. Available balance for spending
	This is the balance that the user can use to spend.

	2. Pending balance
	This is the balance that is pending settlements (eg from refunds, etc)
...
class class_user_wallet extends class_main {
	...
	public function get_user_wallet_balance_amt_shiptokens($user_hash) {

		// Variables
		$total_shiptokens = 0.00;
		$total_topups = 0.00;
		...
		$query_topups = "SELECT
					SUM(`txn-amt_shiptokens`) AS `txn-amt_shiptokens`
				FROM wallet_topups
		...
		try {
			...
			if ($result_topups['txn-amt_shiptokens']) {
				// The total amount of shiptokens
				$total_topups = $result_topups['txn-amt_shiptokens'];
			}
		...
		// TODO $total_withdrawals
		// TODO $total_adjustments

		// Calculate the total shiptokens
		$total_shiptokens = $total_topups - $total_withdrawals + $total_adjustments;

		// Return the total amount of shiptokens
		return $total_shiptokens;
	}


	// Get the user's wallet balance details
	// 1. Available balance for spending = total_topups - total_withdrawals + total_bet_payouts
	// 2. Withdrawable balance = (total_bet_payouts - total_withdrawals) - total_topups (Any topped up shiptokens are not withdrawable)
	public function get_user_wallet_balance_details($user_hash) {
		...
		$user_wallet_balance_amt_shiptokens = $this->get_user_wallet_balance_amt_shiptokens($user_hash);
		...
		return array(
			'available_balance' => $user_wallet_balance_amt_shiptokens,
			'pending_balance' => $result_pending_balance['txn-amt_shiptokens'],
			'withdrawable_balance' => $result_withdrawable_balance['txn-amt_shiptokens'],
		);
	}
	...
	public function get_transaction_history_by_user_hash($user_hash) {
		...
		$query = "SELECT
					...
					`txn-amt_shiptokens` AS `txn-amt_shiptokens`,
	...
	// Attempts to charge the user's wallet for an amount of shiptokens
	public function attempt_charge_user_wallet($user_hash, $amt_shiptokens) {
		...
		// Ensure amt_shiptokens is valid
		if ($amt_shiptokens <= 0.00) {
			$this->add_error('Invalid amount of shiptokens', 'warning');
			return false;
		}
		...
		// Ensure the user has enough shiptokens in their wallet
		if ($user_wallet_balance_amt_shiptokens < $amt_shiptokens) {
			$this->add_error('Insufficient shiptokens in wallet', 'warning');
			return false;
		}
	}
	...
	public function submit_withdrawal_request($user_hash, $amt_shiptokens) {
		...
		// Ensure the user has enough shiptokens in their wallet
		if ($user_wallet_withdrawable_balance < $amt_shiptokens) {
			$this->add_error('Insufficient shiptokens in wallet', 'warning');
			...
			$query_insert_failed_withdrawal = "INSERT INTO wallet_transactions (
												...
												'Insufficient shiptokens in wallet'
												)";

			$query_params_insert_failed_withdrawal = array(
				':userhash' => $user_hash,
				':amtshiptokens' => $amt_shiptokens,
			);
	...
	public function create_topup_transaction_request($amt_shiptokens, $amt_USD_cents, $gateway, $user_hash = null) {
		...
		// The amount of shiptokens must be valid
		if ($amt_shiptokens <= 0.00) {
			$this->add_error('Invalid amount of shiptokens', 'warning');
			return false;
		}
